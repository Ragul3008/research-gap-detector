...
def detect_hallucinations(
    ...
    Args:
        ...
        gap_data         : Optional gap analysis results.
    ...
    # ── Check for claimed gaps ──────────────
    if gap_data and "gap_dimensions" in gap_data:
        auditable = gap_data.get("auditable_formulas", {})
        item_to_coverage = {}
        for dim in auditable:
            for x in auditable[dim].get("dimensions_log", []):
                item_to_coverage[x["item"].lower()] = x.get("avg_coverage", 0.0)
                
        gap_indicators = ["gap", "lack", "overlook", "underexplored", "underrepresented", "absent", "scarc", "rare", "omitted", "no studies"]
        
        from models.gap_engine import (
            REGION_TAXONOMY, POPULATION_TAXONOMY, METHOD_TAXONOMY,
            THEME_TAXONOMY, THEORY_TAXONOMY, TIME_TAXONOMY
        )
        all_taxonomy_items = (
            REGION_TAXONOMY + POPULATION_TAXONOMY + METHOD_TAXONOMY +
            THEME_TAXONOMY + THEORY_TAXONOMY + TIME_TAXONOMY
        ...
        for claim in claims:
            claim_lower = claim.lower()
            has_gap_word = any(ind in claim_lower for ind in gap_indicators)
            if not has_gap_word:
                continue
                
            for item in all_taxonomy_items:
                item_lower = item.lower()
                if item_lower in claim_lower:
                    avg_cov = item_to_coverage.get(item_lower, 0.0)
                    if avg_cov > 0.5:
                        flagged_claim = {
                            "claim": claim,
                            "supported": False,
                            "confidence": 0.0,
                            "issue": f"Claimed gap in '{item}' contradicts evidence; it is widely covered in the corpus (coverage={avg_cov:.2f}).",
                            "severity": SEVERITY_HIGH,
                            "matched_paper": None
                        }
                        claim_results.append(flagged_claim)
